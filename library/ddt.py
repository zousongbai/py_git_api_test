...
def ddt(cls):  # 接收测试用例类
    ...
    for name, func in list(cls.__dict__.items()):
        # (1)cls.__dict__.items()：获取所有的类属性和方法，然后进行字典形式打包，然后.items()进行遍历
        # (2)name：属性的名字或方法的名字
        # (3)func:属性的值或方法的值
        if hasattr(func, DATA_ATTR):  # 判断func方法有没有DATA_ATTR的属性

            for i, v in enumerate(getattr(func, DATA_ATTR)):
                test_name = mk_test_name(name, getattr(v, "__name__", v), i)
                # test_data_docstring = _get_test_data_docstring(func, v)#本身原有的
